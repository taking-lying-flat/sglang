...
mod tests {
    ...
    /// Guards `std::env::set_var` tests against
    /// `input_ids_offload_on_by_default_and_gated_by_flag`: env is
    /// process-global and tests run in parallel, so any test that sets
    /// SGLANG_ROUTER_DISABLE_INPUT_IDS_OFFLOAD must hold this lock across
    /// both the set and the parse (and every test asserting the env-absent
    /// default must hold it too). Poison-tolerant via [`lock_env`]: a
    /// panicking test must neither leak the var nor cascade-fail its
    /// siblings with a misleading PoisonError.
    ...
    fn lock_env() -> (std::sync::MutexGuard<'static, ()>, EnvReset) {
    ...
    fn input_ids_offload_on_by_default_and_gated_by_flag() {
        ...
        let on =
            into_config_owned(with_model(&["--worker-urls", "http://10.0.0.1:30000"])).unwrap();
        assert!(
            on.model.forward_input_ids,
            "the input_ids offload must default to enabled"
        );
        let off = into_config_owned(with_model(&[
            "--worker-urls",
            "http://10.0.0.1:30000",
    ...
    fn input_ids_offload_gated_by_boolish_env_var() {
        ...
        for value in ["", "banana", " 1"] {
            std::env::set_var("SGLANG_ROUTER_DISABLE_INPUT_IDS_OFFLOAD", value);
            assert!(
                into_config_owned(with_model(&["--worker-urls", "http://10.0.0.1:30000"])).is_err(),
